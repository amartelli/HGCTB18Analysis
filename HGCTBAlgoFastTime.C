...
public :
  ...
  void   Init(std::vector<float>& rechit_energy, std::vector<unsigned int>& rechit_layer,
	      ...
	      TGraph* timeZ, TH2F* timeZFrom0);
  float GetTime(int layer, int extend, TH1F* timeHisto,  TH1F* timeHistoAll, TH2F* timeVsEnergy);
  float GetTimeSmeared(int layer, int extend, TH1F* timeHisto, TH2F* timeVsEnergy);
  float SmearTime(float energy, int moduleId);

private :
  bool  debug=false;  
  vector< vector<float> > layer_rechit_en; 
  vector< vector<float> > layer_rechit_MIP; 
...
void HGCTBAlgoFastTime::Init(std::vector<float>& rechit_energy, std::vector<unsigned int>& rechit_layer,
  ...
  layer_rechit_MIP.resize(maxLayer);  
  ...
  std::vector<float> firstTimesLayer(maxLayer, 999);
  std::vector<float> zedLayer(maxLayer, 0);

  for (unsigned int irechit=0; irechit < totLayerHits; ++irechit){
    
    int moduleId = rechit_module[irechit]; 
    float Mip2GeV = (85.5e-6); //300um
    if(moduleId == 144 || moduleId == 145 || moduleId == 146 || moduleId == 147) Mip2GeV =  (57e-6); //200um
    float nMip = rechit_energy[irechit];

    //set to 15MIP + cut on timing (redundant but ok)
    if(nMip < cut || rechit_time_mc[irechit] < -90.) continue;
    float rhEnergy = nMip * Mip2GeV;
    ...
    if(debug) std::cout << " timeHit = " << timeHit << " rhEnergy  = " << rhEnergy << " nMip = " << nMip << std::endl;

    //to build profile with time vs z 
    if(timeHit < firstTimesLayer[layer-1]) firstTimesLayer[layer-1] = timeHit;
    zedLayer[layer-1] = rechit_z[irechit];
    //other option
    timeZFrom0->Fill(rechit_z[irechit], timeHit - 3268.5/speed_light);

    float effectivedZ = 3268.5; // offset on negative side cm beam to HGCAL

    //just care of dz distance (3D tof seem to perform worst)
    float dist3Dhit = rechit_z[irechit]; 
    float tofHit = (dist3Dhit + effectivedZ)/speed_light;
    
    // if(debug)  std::cout << " dist3Dhit = " << dist3Dhit << " tof = " << tofHit << " timeHit = " << timeHit 
    //  			 << " zFirstLayer = " << zFirstLayer << std::endl;

    timeHit = timeHit - tofHit; 

    float smearing = SmearTime(nMip, moduleId);
    float timeHitSmeared = ran->Gaus(timeHit, smearing);
    if(debug) std::cout << " timeHit  = " << timeHit << " timeHitSmeared = " << timeHitSmeared 
			<< " energy = " << rhEnergy << " nMip = " << nMip << std::endl;

    if(debug) std::cout << " layer = " << layer << std::endl;

    layer_rechit_en[layer-1].push_back(rhEnergy);
    layer_rechit_MIP[layer-1].push_back(rechit_energy[irechit]);
  ...
  //fill profile with firstTime_per layer (using time_15MIP threshold!!) vs z 
  int iBin = 0;
  for(auto ij:firstTimesLayer){
    //    std::cout << " filling histo => z = " << zedLayer[iBin] << " time = " << ij << std::endl; 
    timeZ->SetPoint(iBin+1, zedLayer[iBin], ij);
    ++iBin;
  }
  ...
  if(debug) std::cout << " end main loop " << std::endl;
}


// energy weighted time
float HGCTBAlgoFastTime::GetTime(int layer, int extend, TH1F* timeHisto, TH1F* timeHistoAll, TH2F* timeVsEnergy){
  if(debug) std::cout << " hello in GetTime layer = " << layer << std::endl;

  float maxX;
  float maxY;
  float maxE = 0.;

  //for each call identify the range of layers of interest
  ...
  //for each call with identified maxE hit and layer just select the hits in the proper clustering region (R1, R2...)
  ...
  for(unsigned int ij = startLayer; ij<endLayer; ++ij){
    ...
    for(unsigned int iR=0; iR<size; ++iR){
      ...
      //plot with time distrib for all hits in the layer
      float time = layer_rechit_time[ij-1][iR];
      timeHistoAll->Fill(time);
      ...
      //just plot the time of hits in the clustering region
      timeHisto->Fill(time);
      energyTot += en;
    }
  }
  ...
  if(times.size() >= 3){
    ...
    if(averageTime != -99 ) timeVsEnergy->Fill(energyTot, averageTime);
    return(averageTime);
  }
  else {
...
////
// energy weighted time smeared examtly as above but picking the smeared time values
float HGCTBAlgoFastTime::GetTimeSmeared(int layer, int extend, TH1F* timeHisto, TH2F* timeVsEnergy){
  if(debug) std::cout << " hello in GetTimeSmeared layer = " << layer << std::endl;
...
float HGCTBAlgoFastTime::SmearTime(float energy, int moduleId){
  ...
  // values from the "electronics" ab measurements (A = 4ns/Q and C = 50ps)
  // float stoc_smear_time_300 =  1.04;
  // float stoc_smear_time_200 =  1.56;

  //"more realistic" values from fit to 1 channel by Artur
  // => to be cross-checked with proper analysis of data
  float stoc_smear_time_300 =  10.42;
  float stoc_smear_time_200 =  15.62;
